...
One can prove that 
the natural map $\B_n\to\B_{n+1}$ is an injective group homomorphism, this is not an easy proof. Moreover,
the diagram
\[\begin{tikzcd}
	\B_n & \Sym_n \\
	\B_{n+1} & \Sym_{n+1}
...
is left-ordered. The proof of this fact is quite hard. However, 
...
Burns--Hale's theorem states that a group $G$ is left-ordered if and only if
