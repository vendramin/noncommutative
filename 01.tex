...
A subgroup $H$ of $G$ is said to be \emph{characteristic} if 
...
is using character theory. % See the course
...
% There are several generalizations, see the notes. There is one, we want to mention here. 
...
\emph{elementary abelian} if $x^p=1$ for all $x\in P$.
\end{definition}

\begin{definition}
\index{Subgroup!minimal normal}
A subgroup $M$ of $G$ is said to be \emph{minimal normal} if $M\ne\{1\}$
(or $G = \{1\}$),
